<?php
    ...
    //page name
    $pgnm='Co- Accomodation: Dashboard';
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <?php if (is_tenant_user() && $currentTenant) {
                    ...
                    $myInvoices = $myInvoicesResult ? (int) mysqli_fetch_assoc($myInvoicesResult)['total'] : 0;
                    $myComplaintsResult = mysqli_query($connection, "SELECT COUNT(*) AS total FROM `tenant_complaints` WHERE `tenant_id`='$tenantId'");
                    $myComplaints = $myComplaintsResult ? (int) mysqli_fetch_assoc($myComplaintsResult)['total'] : 0;
                    $availablePartitionsResult = mysqli_query($connection, "SELECT hp.*, h.house_name, h.location FROM `house_partitions` hp LEFT JOIN `houses` h ON hp.`house_id`=h.`houseID` WHERE hp.`partition_status`='Vacant' ORDER BY h.`house_name`, hp.`partition_number`");
                    $currentPartitionResult = $partitionId > 0 ? mysqli_query($connection, "SELECT hp.*, h.house_name, h.location FROM `house_partitions` hp LEFT JOIN `houses` h ON hp.`house_id`=h.`houseID` WHERE hp.`partition_id`='$partitionId' LIMIT 1") : false;
                    $currentPartition = $currentPartitionResult && mysqli_num_rows($currentPartitionResult) === 1 ? mysqli_fetch_assoc($currentPartitionResult) : null;
                ?>
                <div class="row">
                    <div class="col-md-4 col-sm-6">
                        <div class="white-box">
                            <h4>My Stay</h4>
                            <p><strong>House:</strong> <?php echo htmlspecialchars((string) $currentTenant['house_name'], ENT_QUOTES, 'UTF-8'); ?></p>
                            <p><strong>Partition:</strong> <?php echo htmlspecialchars((string) $currentTenant['partition_number'], ENT_QUOTES, 'UTF-8'); ?></p>
                            <p><strong>Rent:</strong> <?php echo htmlspecialchars((string) $currentTenant['rent_amount'], ENT_QUOTES, 'UTF-8'); ?></p>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-6">
                        <div class="white-box">
                            <h4>My Invoices</h4>
                            <h2><?php echo $myInvoices; ?></h2>
                    ...
                    <div class="col-md-4 col-sm-6">
                        <div class="white-box">
                            <h4>My Complaints</h4>
                            <h2><?php echo $myComplaints; ?></h2>
                            <p><a href="complaints.php">Track complaint status</a></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="white-box">
                            <h4>Facilities In My Partition</h4>
                            <?php
                                if ($currentPartition) {
                                    echo render_partition_facilities_badges(isset($currentPartition['facilities']) ? $currentPartition['facilities'] : '');
                                } else {
                                    echo '<span class="text-muted">No facilities listed yet.</span>';
                                }
                            ?>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="white-box">
                            <h4>Available Partitions</h4>
                            <?php
                                if (!$availablePartitionsResult || mysqli_num_rows($availablePartitionsResult) === 0) {
                                    echo '<i style="color:brown;">No vacant partitions available right now.</i>';
                                } else {
                                    echo '<div style="max-height:260px; overflow:auto;">';
                                    while ($partition = mysqli_fetch_assoc($availablePartitionsResult)) {
                                        echo '<div style="border-bottom:1px solid #e4e7ea; padding:8px 0;">';
                                        echo '<strong>'.htmlspecialchars($partition['house_name'], ENT_QUOTES, 'UTF-8').' - '.htmlspecialchars($partition['partition_number'], ENT_QUOTES, 'UTF-8').'</strong><br>';
                                        echo 'Rent: '.htmlspecialchars((string) $partition['rent_amount'], ENT_QUOTES, 'UTF-8').'<br>';
                                        echo 'Location: '.htmlspecialchars((string) $partition['location'], ENT_QUOTES, 'UTF-8').'<br>';
                                        echo render_partition_facilities_badges(isset($partition['facilities']) ? $partition['facilities'] : '');
                                        echo '</div>';
                                    }
                                    echo '</div>';
                                }
                    ...
                    <div class="col-md-12 col-lg-6 col-sm-12">
                        <div class="white-box">
                            ...
                            <div class="row sales-report">
                                <div class="col-md-6 col-sm-6 col-xs-6">
                                    <h2>Co- Accomodation  <?php echo 2000+date('y'); ?></h2>
                                    <p>Blog Posts</p>
                                </div>
                                <div class="col-md-6 col-sm-6 col-xs-6 ">
