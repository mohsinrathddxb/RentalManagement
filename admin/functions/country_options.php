...
function get_flag_emoji($isoCode) {
    $isoCode = strtoupper((string) $isoCode);
    if (strlen($isoCode) !== 2 || preg_match('/[^A-Z]/', $isoCode)) {
        return '';
    }

    $flag = '';
    for ($index = 0; $index < 2; $index++) {
        $flag .= mb_chr(127397 + ord($isoCode[$index]), 'UTF-8');
    }

    return $flag;
}

function get_country_options($selectedCountry = '') {
    $options = '<option value="">** Select country **</option>';
    foreach (get_country_records() as $country) {
        $safeCountry = htmlspecialchars($country['name'], ENT_QUOTES, 'UTF-8');
        $selected = strcasecmp($country['name'], (string) $selectedCountry) === 0 ? ' selected' : '';
        $options .= '<option value="'.$safeCountry.'"'.$selected.'>'.$safeCountry.'</option>';
    }

    return $options;
}

function get_phone_code_options($selectedCode = '+971') {
    $options = '<option value="">Code</option>';
    foreach (get_country_records() as $country) {
        $safeCode = htmlspecialchars($country['code'], ENT_QUOTES, 'UTF-8');
        $safeName = htmlspecialchars($country['name'], ENT_QUOTES, 'UTF-8');
        $flag = get_flag_emoji($country['iso']);
        $selected = $country['code'] === $selectedCode ? ' selected' : '';
        $options .= '<option value="'.$safeCode.'"'.$selected.'>'.$flag.' '.$safeCode.' '.$safeName.'</option>';
    }

    return $options;
}

function split_phone_number($phoneNumber) {
    $phoneNumber = trim((string) $phoneNumber);
    if ($phoneNumber === '') {
        return ['code' => '+971', 'number' => ''];
    }

    $records = get_country_records();
    usort($records, function ($left, $right) {
        return strlen($right['code']) <=> strlen($left['code']);
    });

    foreach ($records as $country) {
        if (strpos($phoneNumber, $country['code']) === 0) {
            return [
                'code' => $country['code'],
                'number' => trim(substr($phoneNumber, strlen($country['code'])))
            ];
        }
    }

    return ['code' => '+971', 'number' => $phoneNumber];
}
